...
class Wsu_Mediacontroll_Test_Model_System_Config_Adapter extends EcomDev_PHPUnit_Test_Case {
    ...
    public function setUp() {
        $this->_model = Mage::getModel('wsu_mediacontroll/system_config_adapter');
    }
    public function testConfigurationNotSavedIfModuleNotInstalled() {
        $imagemagickMock = $this->getMock('Varien_Image_Adapter_Imagemagic', array(
            'checkDependencies'
        ));
        $imagemagickMock->expects($this->any())->method('checkDependencies')->will($this->throwException(new Exception()));
        $sessionMock = $this->getModelMock('adminhtml/session', array(), false, array(), '', false);
        $sessionMock->expects($this->once())->method('addError')->withAnyParameters();
